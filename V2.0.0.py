...
#Fonction réalisant une action lorsque l'on clique sur une case
    ...
    if Bse_donnee[x].type == "Bombe":
        Bse_Case[x].config(image = ImageBombe, state = "disabled")
        Perdu = showwarning("Fin du game", message = "Vous avez perdu", default = "ok")
    else:
        n = 0
        while n != Bse_donnee[x].type:
            n += 1
        Bse_Case[x].config(image = ImagesNuméros[n], state = "disabled")
    CasesRévélées += 1
...
for ligne in range(10):
    for colonne in range(10):
        ...
        Bse_Case[Compteur] = Button(TableauDemineur, image = ImageBlanc, state = "active", command = lambda Compteur=Compteur:clic(Bse_donnee[Compteur].compteur))
